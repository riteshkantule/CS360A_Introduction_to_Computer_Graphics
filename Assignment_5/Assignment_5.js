...
vec4 phongColor(Sphere sphere, float t, Ray ray)
{
  ...
  vec3 L = normalize(lightPos - coords);
  vec3 E = normalize(cameraPos - coords);
...
void main()
{
  ...
  sphere[1].center = vec3(1.0, 0.15, 1.5);
  sphere[1].radius = 0.5;
  sphere[1].color = vec3(0.1, 0.2, 0.8);
  sphere[1].shine = 50.0;

  sphere[2].center = vec3(-1.0, 0.15, 1.5);
  sphere[2].radius = 0.5;
  sphere[2].color = vec3(0.1, 0.8, 0.2);
  sphere[2].shine = 5.0;
  ...
  for(int i = 0; i < sphere.length(); i++)
  {
    float t = intersectSphere(ray, sphere[i]) + bias;
    if(t >= 0.0 && t < tmin)
    {
      tmin = t;
      index = i;
...
function rotateLight()
{
  ...
  lightPos[0] = 4.0 * Math.sin(angle);
  lightPos[2] = 4.0 * Math.cos(angle);
...
function webGLStart() {
  ...
  uCameraPositionLocation = gl.getUniformLocation(shaderProgram, "cameraPos");
  ...
  gl.uniform3fv(uCameraPositionLocation, cameraPos);
